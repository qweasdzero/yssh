...
namespace UnityGameFramework.Runtime
{
    ...
    public static class Helper
    {
        ...
        /// <param name="helperTypeName">要创建的辅助器类型名称。</param>
        /// <param name="customHelper">若要创建的辅助器类型为空时，使用的自定义辅助器类型。</param>
        /// <param name="instanceRoot">创建的辅组器的游戏物体。</param>
        /// <returns>创建的辅助器。</returns>
        public static T CreateHelper<T>(string helperTypeName, T customHelper, GameObject instanceRoot)
            where T : MonoBehaviour
        {
            return CreateHelper(helperTypeName, customHelper, 0, instanceRoot);
        }
        ...
        {
            return CreateHelper(helperTypeName, customHelper, index, null);
        }
        ...
        /// <param name="instanceRoot">创建的辅组器的游戏物体。</param>
        /// <returns>创建的辅助器。</returns>
        public static T CreateHelper<T>(string helperTypeName, T customHelper, int index, GameObject instanceRoot)
            where T : MonoBehaviour
        {
            T helper = null;
            if (!string.IsNullOrEmpty(helperTypeName))
            {
                ...
                {
                    instanceRoot = new GameObject();
                }

                helper = (T) instanceRoot.AddComponent(helperType);
            }
