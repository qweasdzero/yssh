...
{
    public class APPWindow : GameFrameworkMeumWindow
    {
        ...
        void DrawGUI()
        {
            ...
            EditorGUILayout.BeginHorizontal(EditorStyles.toolbar);
            {
                ...
                {
                    string refreshTooltip = "Should all entries be automaticly refreshed every " +
                    ...
                    for (int i = 0; i < filteredPpeList.Count; i++)
                    {
                        if (filteredPpeList[i].Value != null)
                        {
                            EditorGUILayout.BeginHorizontal();
                            {
                                ...
                                GUIStyle numberFieldStyle = filteredPpeList[i].HasChanged
                                ...
                                switch (filteredPpeList[i].Type)
                                {
                                    ...
                                    case ValueType.String:
                                        filteredPpeList[i].Value = EditorGUILayout.TextField("",
                                            (string) filteredPpeList[i].Value, numberFieldStyle,
        ...
        private void Export(bool onlySelected)
        {
            ...
            {
                string exportPath = EditorUtility.SaveFilePanelInProject("Export all PlayPrefs entries",
        ...
        {
            string plistPath = Environment.GetFolderPath(Environment.SpecialFolder.Personal) +
 ...
 cuKey.CreateSubKey("Software\\AppDataLow\\Software\\" + PlayerSettings.companyName + "\\" + PlayerSettings.productName);
        ...
        {
            string path = Environment.GetFolderPath(Environment.SpecialFolder.Personal) + "/.config/unity3d/" +
        ...
        private bool IsUnityWritingToPlist()
        {
            ...
            if (Application.platform == RuntimePlatform.OSXEditor)
            {
                ...
                FileInfo plistFile = new FileInfo(Environment.GetFolderPath(Environment.SpecialFolder.Personal) +
                                                  "/Library/Preferences/unity." + PlayerSettings.companyName + "." +
                                                  PlayerSettings.productName + ".plist");
...
//                    path = path.Substring(0, path.LastIndexOf('/') + 1);
                    ...
                    iconsPath = Path.Combine(path, "Icons/");
                }
        ...
        {
            get { return GetStyle("ToolbarSeachTextField"); }
        }

        private GUIStyle ToolbarSeachTextFieldPopup
        {
            get { return GetStyle("ToolbarSeachTextFieldPopup"); }
        }

        private GUIStyle ToolbarSeachCancelButton
        {
            get { return GetStyle("ToolbarSeachCancelButton"); }
        }

        private GUIStyle ToolbarSeachCancelButtonEmpty
        {
            get { return GetStyle("ToolbarSeachCancelButtonEmpty"); }
        }
