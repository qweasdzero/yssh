    ...
    public class LinuxPreferenceProvider : BasePreferenceProvider
    {
            ...
            get
            {
                if (string.IsNullOrEmpty(s_EditorPrefsPath))
                {
                    ...
                    if (string.IsNullOrEmpty(prefix))
                        prefix = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Personal),
                            ".local/share");
        ...
        public override void SetKeyValue(string valueName, object newValue)
        {
            ...
            XmlElement oldElement =
                (XmlElement) prefs.SelectSingleNode(string.Format("/unity_prefs/pref[@name='{0}']", valueName));
            if (oldElement == null)
            {
        ...
        public override void FetchKeyValues(IDictionary<string, object> prefsLookup)
        {
            ...
            foreach (XmlElement pref in prefs.SelectNodes("/unity_prefs/pref").OfType<XmlElement>())
            {
                try
                {
                    prefsLookup[pref.Attributes["name"].Value] =
                        ParseValue(pref.Attributes["type"].Value, pref.InnerText);
                }
                catch (Exception e)
                {
